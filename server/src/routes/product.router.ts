...
productRouter.post('/newProductImages', uploadImages.array('image'), async (req: Request, res: Response) => {
    try {
        let filesToUpload: Express.Multer.File[] = req.files as Express.Multer.File[]
        if (!filesToUpload || filesToUpload.length < 0) throw new Error(`Files not available for upload`)
...
productRouter.get('/getOne/:productId', async (req: Request, res: Response) => {
    ...
    try {
        ...
        LOG.error(error)
        res.status(500).json({ error: `Unable to find matching document with productId: ${productId}` });
    }
})
