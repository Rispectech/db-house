...
export function App() {
    ...
    let checkJWT = () => {
        return new Promise((resolve, reject) => {
            ...
            if (!jwt || !utype) {
                ...
                fetch(Rest + "/auth/verifyAJwt", { method: 'post', headers }).then(res => {
                    if (res.status === 200) {
                        return res;
                ...
                let headers = { 'Content-Type': 'application/json', "Authorization": "Bearer " + jwt }
                fetch(Rest + "/auth/verifyMJwt", { method: 'post', headers }).then(res => {
                    if (res.status === 200) {
                        return res;
                    } else throw new Error(`Invalid JWT`)
                }).then(res => {
    ...
    useEffect(() => {
        checkJWT().then(() => {
            RestClient.getCategories().then((result) => {
    ...
    return (
        <Router>
            <Routes>
                ...
                <Route path="merchant/ProductList2" element={<ProductList2 />} />
